...
- Users can request a complete replacement script in plain language from a current, non-stale plan. The replacement may change any canonical script field and is validated before it replaces the current script.
...
- Script revision requests, prior model output, source text, and plans remain session-only and are excluded from render jobs and exports. Existing completed audio remains available after a revision and is marked stale against the replacement script.
